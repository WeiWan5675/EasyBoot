...
public class RestTemplateCustomConfiguration {
    ...
    public HttpClientConnectionManager poolingConnectionManager() {
        ...
        poolingConnectionManager.setMaxTotal(bootProperties.getHttp().getMaxTotal()); // 连接池最大连接数
        poolingConnectionManager.setDefaultMaxPerRoute(bootProperties.getHttp().getMaxPerRoute()); // 每个主机的并发
        // 长连接
        poolingConnectionManager.setDefaultSocketConfig(SocketConfig.custom()
            .setSoTimeout(bootProperties.getHttp().getSocketTimeout()).setSoKeepAlive(true).build());
        // 连接不活跃多久检查毫秒 并不是100 % 可信
        poolingConnectionManager.setValidateAfterInactivity(bootProperties.getHttp().getValidateAfterInactivity());
        // 空闲扫描线程
        HttpClientIdleConnectionMonitor.registerConnectionManager(poolingConnectionManager,
            bootProperties.getHttp().getIdleTimeToDead(), bootProperties.getHttp().getIdleScanTime());
    ...
    public HttpClientBuilder httpClientBuilder(SSLContext sslContext) {
        ...
        RequestConfig requestConfig = RequestConfig.custom()
            .setConnectionRequestTimeout(bootProperties.getHttp().getConnectionRequestTimeout())// 获取连接等待时间
            .setConnectTimeout(bootProperties.getHttp().getConnectTimeout())// 连接超时
            .setSocketTimeout(bootProperties.getHttp().getSocketTimeout())// 获取数据超时
            .build();
        httpClientBuilder.setDefaultRequestConfig(requestConfig).setSSLContext(sslContext)
            .setUserAgent(bootProperties.getHttp().getUserAgent()).disableContentCompression()
            // 收动设置连接池 这个在连接池那里设置，查看该方法注释该,为了避免版本升级导致错误，也冗余设置
            .setConnectionTimeToLive(bootProperties.getHttp().getConnTimeToLive(), TimeUnit.MILLISECONDS)// 连接最大存活时间
            // 默认重试次数为0，POST 不重试
            .setRetryHandler(new StandardHttpRequestRetryHandler(bootProperties.getHttp().getRetyTimes(), false));
