...
public class WebConfigurer implements WebMvcConfigurer {
    ...
    /**
     * 跨域很常见，默认框架参数开启，如果想更安全，请设置allowedOrigins，如https://www.easyboot.com
     * <p>
    ...
    public void addCorsMappings(CorsRegistry registry) {
        BootProperties.CorsProperties cors = bootProperties.getCors();
        if (cors.isEnable()) {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        // 添加数据权限ThreadLocal清理, 数据权限通过在ThreadLocal中保存切入点注解信息
        DataAuthorityClearInterceptor dataAuthorityClearInterceptor = SpringContextHolder.getBean(DataAuthorityClearInterceptor.class);
        if (dataAuthorityClearInterceptor != null) {
            if (bootProperties.getDataAuthority().isEnable()) {
                registry.addInterceptor(dataAuthorityClearInterceptor).
                        addPathPatterns("/**");
            }
        }
    }
